package net.zaharenko424.casualties_cubed.compat.simple_voicechat;
...
import net.zaharenko424.casualties_cubed.network.packet.ServerboundTalkPacket;
...
public class MainVoicePlugin implements VoicechatPlugin {
    ...
    private boolean sendDistorted(Player player) {
        return player.getCapability(PlayerHealthProvider.PLAYER_HEALTH_DATA).map(data -> data.getLimb(Limb.HEAD).dislocationTimer() > 0 || data.disfigured()).orElse(false);
    }
