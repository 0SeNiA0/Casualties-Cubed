...
public class PlasticDressingItem extends Item implements IBandage, IAllowInMedicBags {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        ...
        pTooltipComponents.add(Component.translatable("item.casualties_cubed.plastic_dressing.description").withStyle(ChatFormatting.GRAY));
    }

    @Override
    public void useBandageAction(float scalableAmount, Player target, @Nullable Limb limb) {
        target.getCapability(PlayerHealthProvider.PLAYER_HEALTH_DATA).ifPresent(data -> {
            data.addDelayedChange(((0.02f * scalableAmount) / 20f) / 60f, 200, limb);
