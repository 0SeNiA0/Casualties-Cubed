...
@Mod.EventBusSubscriber(modid = CasualtiesCubed.MOD_ID, value = Dist.CLIENT,bus = Mod.EventBusSubscriber.Bus.MOD)
    ...
    public static void registerShaders(RegisterShadersEvent event) throws IOException {
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("consciousness"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> CONSCIOUSNESS_SHADER = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("pain_vignette"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> PAIN_SHADER = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("leftblind"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> LEFT_EYE_BLIND = shader
        );event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("rightblind"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> RIGHT_EYE_BLIND = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("fullblind"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> FULL_BLIND = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("warp_chromaabb"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> WARP_CHROMAABB = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("sickness"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> SICKNESS = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("heat"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> HEAT = shader
        );
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(),
                        CasualtiesCubed.resourceLoc("cold"),
                        DefaultVertexFormat.POSITION_TEX),
                shader -> COLD = shader
        );
    }
