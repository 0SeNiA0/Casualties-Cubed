...
public class ClientEvent {
    ...
    public static void onContainerDraw(ScreenEvent.Render event) {
        ...
        if (carried.getItem() instanceof MultiTankFluidItem from && hovered.getItem() instanceof MultiTankFluidItem to){
            if (from.getHandler(carried).getTank().getTotalFluid() == 0 || to.getHandler(hovered).getTank().getFreeSpace() == 0) return;

            GuiGraphics guiGraphics = event.getGuiGraphics();
            int mouseX = event.getMouseX();
            int mouseY = event.getMouseY();
            Component text = Component.translatable("casualties_cubed.gui.fluid_screen", Component.keybind(Minecraft.getInstance().options.keyUse.getName()));
